...
From lrust.typing.lib.slice Require Import iter uniq_slice.
...
Section code.
  ...
  Lemma inc_vec_type
    : typed_val (inc_vec)
      (fn<α>(∅; &uniq{α} (vec_ty int)) → ())
      (λ post '-[(v, v')],
        v' = map (λ e, e + 5) v → post ()
      ).
  Proof.
    eapply type_fn; [apply _|]=>/= α ϝ ret [v[]]. simpl_subst. via_tr_impl.
    iApply type_val; [apply (vec_to_uniq_slice_type int)|]. intro_subst.
    iApply type_letcall; [solve_typing|solve_extract|solve_typing|..]. iIntros (iter); simpl_subst.
    iApply (type_cont [] (λ _, +[iter ◁ _]) (λ (post : ()%ST → _) _, post ())). intro_subst.
    iApply (type_cont [] (λ _, +[iter ◁ box (uniq_slice _ int)])
      (λ (post : ()%ST → Prop) '-[v], foldr (λ '(a, a') (b : Prop), a' = a + 5 → b) (post ()) v)
    ). intro_subst.
    - iApply (type_jump (λ _, +[iter ◁ box (uniq_slice _ int)])); [solve_typing| solve_extract| solve_typing|..].
