...
Section token.
  ...
  ([∗ list] (l, aπ) ∈ aπl, l ↦∗: ty.(ty_own) aπ d tid) -∗ q.[κ]
    ={E}=∗ |={E}▷=>^d |={E}=> ∃ξl q', ⌜alapply aπl ./ ξl⌝ ∗ q':+[ξl] ∗
      (q':+[ξl] ={E}=∗
        ([∗ list] (l, aπ) ∈ aπl, l ↦∗: ty.(ty_own) aπ d tid) ∗ q.[κ]).
  Proof.
    ...
    apply proph_dep_constr with (λ x, zip (aπl.*1) x) _ _  in H0.
    iSplit. iPureIntro.
    rewrite /alapply.
    unfold compose in H0. unfold alapply in H0.
  ...
  Proof.
    ...
    iPureIntro. apply proph_dep_constr2. apply proph_dep_constr. done. done. 
  ...
  Proof.
    constructor; rewrite /ghostptrtoken_ty /big_sepAL.
    exact.  
    unfold ty_lfts. eapply ty_lfts_ne. exact. 
    unfold ty_E. eapply ty_E_ne. exact.
    unfold ty_own.
    assert (ty_size x = ty_size y). eapply ty_size_ne. exact.
    rewrite H0.
    intros ????. do 14 f_equiv. apply ty_own_ne; [done|done|done|done|done].
    unfold ty_shr. unfold ghostptrtoken_ty.
    intros ?????. do 10 f_equiv.
