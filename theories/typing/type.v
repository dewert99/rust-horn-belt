...
Section ofe.
  ...
  Global Instance ty_size_proper {A} : Proper ((≡@{_ A}) ==> (=)) ty_size.
  Proof. move=> ?? Eqv. apply Eqv. Qed.
  Global Instance ty_lfts_ne {A} n : Proper ((≡{n}≡@{_ A}) ==> (=)) ty_lfts.
  Proof. move=> ?? Eqv. apply Eqv. Qed.
  Global Instance ty_lfts_proper {A} : Proper ((≡@{_ A}) ==> (=)) ty_lfts.
  Proof. move=> ?? Eqv. apply Eqv. Qed.
  Global Instance ty_E_ne {A} n : Proper ((≡{n}≡@{_ A}) ==> (=)) ty_E.
  Proof. move=> ?? Eqv. apply Eqv. Qed.
  Global Instance ty_E_proper {A} : Proper ((≡@{_ A}) ==> (=)) ty_E.
  Proof. move=> ?? Eqv. apply Eqv. Qed.
  Global Instance ty_outlv_E_ne {A} n :
    Proper ((≡{n}≡@{_ A}) ==> (=) ==> (=)) ty_outlv_E.
  Proof. rewrite /ty_outlv_E. by move=> ?? [_ -> _ _ _]. Qed.
  Global Instance ty_outlv_E_proper {A} :
    Proper ((≡@{_ A}) ==> (=) ==> (=)) ty_outlv_E.
  Proof. rewrite /ty_outlv_E. by move=> ?? [_ -> _ _ _]. Qed.

  Global Instance ty_own_ne {A} n:
    Proper ((≡{n}≡@{_ A}) ==> (=) ==> (=) ==> (=) ==> (=) ==> (≡{n}≡)) ty_own.
  Proof. move=> ?? Eqv ??->??->??->??->. apply Eqv. Qed.
  Global Instance ty_own_proper {A} :
    Proper ((≡@{_ A}) ==> (=) ==> (=) ==> (=) ==> (=) ==> (≡)) ty_own.
  Proof. move=> ?? Eqv ??->??->??->??->. apply Eqv. Qed.
  Global Instance ty_shr_ne {A} n :
    Proper ((≡{n}≡@{_ A}) ==> (=) ==> (=) ==> (=) ==> (=) ==> (=) ==> (≡{n}≡)) ty_shr.
  Proof. move=> ?? Eqv ??->??->??->??->??->. apply Eqv. Qed.
  ...
  Proof.
    ...
    by rewrite Eqv.
  Qed.
  Global Instance ty_of_st_proper {A} : Proper ((≡@{_ A}) ==> (≡)) ty_of_st.
  ...
  Qed.
  Global Instance st_of_pt_proper {A} : Proper ((≡@{_ A}) ==> (≡)) st_of_pt.
  Proof. apply (ne_proper _). Qed.
...
Section traits.
  ...
  Global Instance copy_equiv {A} : Proper ((≡@{_ A}) ==> impl) Copy.
  Proof.
  ...
  Global Instance send_equiv {A} : Proper ((≡@{_ A}) ==> impl) Send.
  Proof. move=> ?? [_ _ _ Eqv _] ?. rewrite /Send=> *. by rewrite -!Eqv. Qed.

  Global Instance sync_equiv {A} : Proper ((≡@{_ A}) ==> impl) Sync.
